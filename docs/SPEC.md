...
録音時はタグを音声へ含めず、タグごとに分割して対応する Cloud TTS 音声で生成した後、1つの MP3 として連結する。現在の採用対象は `ja-JP` と `en-US` で、Chirp3 HD の同一話者名を対応付ける。

| 用途 | ja-JP | en-US |
| --- | --- | --- |
| 学習向け（女性） | Aoede | Aoede |
| ビジネス向け（男性） | Orus | Orus |
| 朗読向け（女性） | Zephyr | Zephyr |
| 朗読向け（男性） | Charon | Charon |

タグのない文章は、録音画面で選んだ基本言語と音声で従来どおり1回だけ生成する。
