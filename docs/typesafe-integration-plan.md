...
### 4.1 Gemini・Jev・既存コードの担当範囲

将来像は「Geminiが作る・直す、Jevが検証済みの項目について問題の有無を判定する」というハイブリッド。ただし、初期導入ではGeminiの判定を残したままJevを並列比較する。現時点ではJevは未実装であり、以下は担当案である。

今回調査した教材生成〜品質チェックの範囲を、関数数やAPI呼び出し回数ではなく「担当する仕事」で数えると、Geminiは生成・修正の5系統、Jevは初期3判定項目、拡張候補まで含めると計12判定項目となる。Jevの12項目すべてを採用・置換すると決定したものではない。

#### Gemini: 生成・修正の5系統

| # | 担当 | 内容 |
| --- | --- | --- |
| 1 | 教材本文の生成 | 元資料や構成に基づき、説明文・例文などを作る。 |
| 2 | クイズの生成 | 問題文、選択肢、正答、解説を作る。 |
| 3 | 不足・不整合の補完と再生成 | 足りない問題の追加、選択肢言語の修正、不備のある教材・問題の作り直し。 |
| 4 | 読み上げ用テキストの生成・最適化 | 表示本文から、読みや言語タグを調整したTTS用テキストを作る。音声そのものの生成とは別。 |
| 5 | 品質問題に対する修正案の生成 | 指摘された本文やTTSについて、具体的な修正文・置換内容を作る。 |

JevのChoice/Score/Noulは代わりの文章や修正差分を生成しないため、これらの生成・修正機能はGemini側に残す。ただし、strict資料モードでのコピー、辞書・ルールによるTTS処理、安全な機械修正など、既存コードだけで処理する経路までGeminiへ移す意味ではない。5系統すべてが毎回Geminiを呼ぶわけではない。

#### Jev: 初期3項目、拡張候補を含めて12項目

| # | 担当する判定 | 導入位置づけ |
| --- | --- | --- |
| 1 | 本文とTTSで意味が変わっていないか | 初期対象。Choiceで意味同等・意味変化・不確実を分類する。 |
| 2 | 仮置き文章・内部メモが教材に残っていないか | 初期対象。Noulで明確な漏れを判定する。 |
| 3 | 伝聞調・冗長さなど、明確な文体上の問題がないか | 初期対象。Scoreで軽微な好みと教材上の問題を分ける。 |
| 4 | 正答と解説が明示的に矛盾していないか | 生成側への拡張候補。修正前の同じ問題を比較する。 |
| 5 | 教材の内容が根拠資料と整合しているか | 根拠資料を渡せる場合のみ。根拠不足も判定結果として扱う。 |
| 6 | 語学教材が、実際に学習対象の表現を扱っているか | 拡張候補。資料の題名・説明だけに終始していないかを補助判定する。 |
| 7 | 選択肢や本文が指定された言語になっているか | 既存の文字種チェックを補完する候補。言語別の実測が必要。 |
| 8 | 読み上げ用テキストに明確な読みの問題が残っていないか | 辞書・読み規約などの根拠が必要。実際の発音の正しさはテキストだけでは証明できない。 |
| 9 | 同じ用語の表記・読みが不統一になっていないか | ファイル内の文脈を渡せる場合の拡張候補。 |
| 10 | TTS変換で意図しない二重読みが生じていないか | 拡張候補。強調などの意図的な反復は除外する。 |
| 11 | TTSの言語区間が意味的に適切か | 拡張候補。タグの形式チェックとは分離する。 |
| 12 | 英文の一部が脱落し、意味が途切れていないか | 探索的な候補。現在の英語spanチェックはログのみである点に注意する。 |

初期の接続先は `_check_pack_quality` の横とし、text/TTSそれぞれに適用できる項目だけを実行する。#4は段階③の生成中レビュー横への拡張候補であり、初期3項目と同時に置き換えるものではない。各項目の質問型・適用制限は本節冒頭の対応表を正とする。

#### 既存コードに残す検査と、比較期間の扱い

JSON形式、必須項目、4択の数、正答番号の範囲、ファイル参照、URL許可、文字数、タグ構造など、決定論的に確定できる検査は既存コードに残す。AIの確率判定で形式・数値・構造の検査を置き換えず、technical validationによる保存ゲートも維持する。

比較期間中は、上記5系統に加えて、既存のGeminiによる本文品質・TTS品質・正答と解説の整合性の3つの判定経路も、それぞれの既存実行条件のまま維持する。Jevの結果は比較ログにのみ記録し、修正実行・再生成・保存判断へ接続しない。人手評価で適性を確認し、別途承認した項目だけを将来の判断補助・ルーティングの対象とする。
