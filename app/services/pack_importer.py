...
def import_pack_files(request: ImportPackRequest) -> ImportPackResponse:
    imported_manifest: PackManifest | None = None
    ...
    for input_file in request.files:
        ...
        if content_type == "pack_manifest":
            imported_manifest = PackManifest.model_validate(input_file.content)
    ...
    plan = CoursePlan(
        ...
        targetUser="imported",
        difficulty="beginner",
        scale=imported_manifest.scale if imported_manifest else None,
    ...
    saved_files = StorageClient().save_files(plan.id, files, metadata.storage_prefix)
    manifest = PackManifest(
        id=f"{metadata.content_id}_manifest",
        contentId=metadata.content_id,
        slug=metadata.slug,
        versionId=metadata.version_id,
        buildId=metadata.build_id,
        generatedAt=metadata.generated_at,
        creator=ManifestCreator(id=metadata.creator_id, displayName=metadata.creator_display_name),
        title=title,
        description=plan.description,
        language=plan.language,
        author=plan.author,
        scale=plan.scale,
        globalTags=imported_manifest.globalTags if imported_manifest else [],
        items=[ManifestItem(kind=file.kind, url=file.url or "") for file in saved_files],
    )
    manifest_file = GeneratedFile(name="manifest.json", kind="manifest", content=manifest.model_dump(exclude_none=True))
    saved_manifest_file = StorageClient().save_files(plan.id, [manifest_file], metadata.storage_prefix)[0]
    validation = validate_files(saved_files, manifest)
    return ImportPackResponse(files=[*saved_files, saved_manifest_file], manifest=manifest, validation=validation, logs=logs)
...
def _resolve_title(request: ImportPackRequest, manifest: PackManifest | None, files: list[GeneratedFile]) -> str:
    if request.title:
