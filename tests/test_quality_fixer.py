...
def test_tts_fix_no_llm_collapses_duplicate_katakana_parenthetical(tmp_path, monkeypatch) -> None:
    ...
    doc["documents"][0]["tts"] = {"text": "ガバナンス（ガバナンス）は重要です。"}
    doc_path.write_text(json.dumps(doc, ensure_ascii=False), encoding="utf-8")
    issue = {
    ...
    data = response.json()
    assert data["unappliedFixes"] == []
    assert data["updatedJson"]["documents"][0]["tts"]["text"] == "ガバナンスは重要です。"
    assert "ガバナンス（ガバナンス）" not in data["updatedJson"]["documents"][0]["tts"]["text"]


def test_tts_fix_no_llm_collapses_concatenated_duplicate_katakana(tmp_path, monkeypatch) -> None:
    target = _write_version(tmp_path, monkeypatch)
    prefix = pack_root_prefix(target["creatorId"], target["contentId"])
    manifest_path = tmp_path / "generated" / prefix / "versions" / target["versionId"] / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    ...
    doc_path = tmp_path / "generated" / prefix / doc_item["url"].split(f"{prefix}/", 1)[1]
    doc = json.loads(doc_path.read_text(encoding="utf-8"))
    doc["documents"][0]["text"] = "Governanceは重要です。"
    doc["documents"][0]["tts"] = {"text": "ガバナンスガバナンスは重要です。"}
    doc_path.write_text(json.dumps(doc, ensure_ascii=False), encoding="utf-8")
    issue = {
        "category": "double_utterance",
        "severity": "medium",
