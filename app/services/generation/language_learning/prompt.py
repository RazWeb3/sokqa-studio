...
def build_language_learning_purpose_lines(plan: CoursePlan) -> list[str]:
    ...
    if (
        ...
        return [
            ...
            "各 documents[] セクションは「短い導入(1文) → 即・学習言語のフレーズを主役として提示 → 短い解説(意味・場面・ニュアンス)」の構成で書くこと。長い日本語メタ説明から始めて第1フレーズまで2〜3段落を消費する構成は禁止する。導入は1文で済ませ、最初の学習言語フレーズをセクションの早い位置(最低でも第1段落内)に置くこと。",
        ]
    return []
