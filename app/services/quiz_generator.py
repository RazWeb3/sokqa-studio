...
from app.services.prompts import (
    quiz_answer_explanation_repair_prompt,
...
def generate_quiz_pack(
    ...
    if get_settings().gemini_provider == "gemini":
        try:
            ...
            # Language Learning only: one conservative semantic review for the
            # entire quiz, followed by a one-time replacement of only the
            # clearly inconsistent questions.  This is intentionally separate
            # from structural validation because it requires semantic judgment.
            content = _repair_answer_explanation_inconsistencies(
                content, plan, quiz_plan, model=model or settings.quiz_model
            )
            pack = SokqaQuizPack.model_validate(content)
...
def _repair_answer_explanation_inconsistencies(
    ...
    *,
    ...
    """Run one batch review and replace only explicitly contradictory questions once."""
    ...
    try:
        issues = batch_quiz_answer_explanation_issues(plan, quiz_plan, content, model=model)
    except Exception as exc:
        # A review outage must not turn a successful generation into a technical
        # failure; the generator prompt still performs its own self-check.
        logger.warning("quiz_generator.consistency_review_failed quiz=%s error=%r", quiz_plan.id, exc)
        return content
    if not issues:
        return content
    ...
    indexes = {
        int(issue.path.split(".")[1])
        for issue in issues
        if issue.path.startswith("questions.") and issue.path.split(".")[1].isdigit()
    }
    targets = [question for index, question in enumerate(questions) if index in indexes]
    target_ids = {str(question.get("id") or "") for question in targets}
    if not targets:
        return content
    logger.warning(
        "quiz_generator.consistency_issues.detected quiz=%s count=%s question_ids=%s classification=quality",
        quiz_plan.id, len(targets), sorted(target_ids),
    )
    try:
        repaired = GeminiClient().generate_json(
            quiz_answer_explanation_repair_prompt(plan, quiz_plan, targets),
            model=model,
            parse_context=LlmJsonParseContext(
                generation_unit="quiz",
                model=model,
    ...
    if not repaired_by_id:
        logger.warning("quiz_generator.consistency_issues.remaining quiz=%s count=%s", quiz_plan.id, len(targets))
        return content
    merged = dict(content)
    merged["questions"] = [
        repaired_by_id.get(str(question.get("id") or ""), question)
        if isinstance(question, dict)
        else question
        for question in content.get("questions") or []
    ]
    # Apply the same field sanitation and answer-position policy as normal
    # generation, while preserving all unaffected questions.
    return normalize_quiz_content(merged, plan, quiz_plan)
