...
- [pack_review.py](file://app/services/pack_review.py)
...
## 更新概要
**変更内容**
- パックレビュー機能の追加により、品質チェックサブシステムにオフライン読み取り専用コンテンツレビュー機能が統合されました
- 新しい pack_review.py モジュールが決定論的なローカル分析を提供し、既存の品質チェックパイプラインを補完しています
- CLI経由でのパックレビュー実行とMarkdownレポート生成機能が追加されました
...
**新機能**: オフラインパックレビュー機能により、ネットワーク接続やLLM呼び出しなしで、ローカルのパックコンテンツに対して決定論的な品質分析を実行できます。これにより、開発環境やCI/CDパイプラインでの迅速な品質チェックが可能になります。
...
PR["pack_review.py<br/>オフラインレビュー"] --> PRF["review_pack_dir<br/>決定論的分析"]
PRF --> IP["ip_check.py<br/>知的財産チェック"]
PRF --> TTS["tts_estimation.py<br/>TTS予測"]
```
...
- [pack_review.py:172-261](file://app/services/pack_review.py#L172-L261)
...
- **pack_review.py**
  - **オフライン読み取り専用パックレビュー：ネットワーク接続やLLM呼び出しなしで決定論的な品質分析を実行。**
  - テキスト校正、TTS予測、知的財産チェック、統計分析などの多角的なレビュー機能を提供。
  - JSON形式とMarkdown形式のレポート生成をサポート。
...
participant PR as "pack_review.py"
API->>QC : 品質チェック要求
QC->>QC : コンテキスト構築
...
QF->>LLM : 必要に応じて修正案生成
QF-->>API : 適用結果・保存準備
Note over PR : オフラインレビュー機能
PR->>PR : review_pack_dir()
PR->>PR : テキスト校正・TTS予測・IPチェック
PR-->>API : 決定論的レビュー結果
```
...
### pack_review.py：**オフライン読み取り専用パックレビュー**
- **review_pack_dir**
  - **ローカルのパックディレクトリを読み取り専用で検査。ネットワーク・課金・書込は一切行わない。**
  - テキスト校正（誤字・重複表現・用語統一）、TTS予測（略語読みの可能性、記号読み、長文検出）、知的財産チェック、統計分析を実行。
  - 既存のvalidation結果を統合し、構造化したレビューレポートを生成。
- **render_review_markdown**
  - **構造化されたレビュー結果を人間が読みやすいMarkdown形式に変換。**
  - XSS対策としてHTMLエスケープを適用し、セキュリティを確保。
- **特徴**
  - **決定論的：同じ入力に対して常に同じ結果を返す。**
  - **オフライン：Gemini/TTS API、ストレージ接続を一切使用しない。**
  - **包括的：構造検証、コンテンツ品質、TTS予測、法的リスクの多角的な分析。**

```mermaid
flowchart TD
Start(["pack_review_dir開始"]) --> Validate["既存validation実行"]
Validate --> LoadPack["pack読み込み"]
LoadPack --> TextCheck["テキスト校正チェック"]
TextCheck --> TTSAnalysis["TTS予測分析"]
TTSAnalysis --> IPCheck["知的財産チェック"]
IPCheck --> Stats["統計分析"]
Stats --> Report["レビューレポート生成"]
Report --> End(["決定論的結果返却"])
```
...
- **pack_review.py は ip_check、packops、tts_estimation、tts_language_tags、tts_sentence_policy に依存し、既存の品質チェックツールを統合利用。**
...
PR["pack_review.py"] --> IP["ip_check.py"]
PR --> PO["packops.py"]
PR --> TE["tts_estimation.py"]
PR --> TL["tts_language_tags.py"]
PR --> TS["tts_sentence_policy.py"]
```
...
- **pack_reviewは完全にオフラインで動作するため、ネットワーク遅延やAPI制限の影響を受けない。**
- **決定論的アルゴリズムにより、大規模パックでも安定したパフォーマンスを発揮。**
...
- **pack_review関連の問題**
  - **pack_reviewはオフライン機能のため、ネットワーク接続や外部APIの失敗には影響されない。**
  - **review_pack_dirはローカルファイルのみを読み取るため、権限エラーが発生する場合はファイルアクセス権限を確認。**
  - **max_findingsパラメータで所見数を制限可能。デフォルト200件で、超過分はsummary.truncatedで通知。**
...
## 結論
品質チェック・自動修正サブシステムは、共通構造検証を基盤とし、Standard と言語学習で明確に分岐する設計を採用しています。言語学習向け戦略は決定論的チェックを重視し、TTS タグと学習フレーズの整合性を保証します。自動修正は安全な範囲で自動適用し、LLM による提案は慎重にフィルタリングして適用します。

**新機能として追加されたオフラインパックレビュー機能は、ネットワーク接続やLLM呼び出しなしで決定論的な品質分析を実現し、開発環境やCI/CDパイプラインでの迅速な品質チェックを可能にします。これにより、教材生成の品質を一貫して担保しつつ、運用負荷をさらに低減できます。**